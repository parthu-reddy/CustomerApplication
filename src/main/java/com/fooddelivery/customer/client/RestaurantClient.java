...
public interface RestaurantClient {
    ...
    @Cacheable(value = "nearbyRestaurants", key = "#lat + '-' + #lng + '-' + #radius", sync = true)
    @GetMapping("/api/v1/restaurants/nearby")
    ApiResponse<List<Object>> getNearbyRestaurants(@RequestParam("lat") double lat, 
                                                   @RequestParam("lng") double lng, 
                                                   @RequestParam("radius") double radius);

    @Cacheable(value = "brandOutlets", key = "#brandId + '-' + #lat + '-' + #lng + '-' + #radius", sync = true)
    ...
    @Cacheable(value = "restaurantDetails", key = "#id", sync = true)
    @GetMapping("/api/v1/restaurants/{id}")
    Map<String, Object> getRestaurantById(@PathVariable("id") UUID id);

    @Cacheable(value = "menuItemsBatch", key = "#id + '-' + #ids", sync = true)
